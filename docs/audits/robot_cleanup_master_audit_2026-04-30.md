...
- Move-only mismatch cleanup started after this proof point: extracted pending-IEA deferral state and `ObservePendingIeaDefer` into `Execution/MismatchEscalationCoordinator.PendingIea.cs`; linked the new shared partial into the NinjaTrader project; core and RobotCore builds passed with `0 Error(s)`; focused checks passed: `AUTHORITY_CONTRADICTIONS`, `ORDER_RECONCILIATION`, `RUN_SUMMARY`, `RUN_SUMMARY_BUILDER`, `EXECUTION_CONTEXT_CONTRACT`.
...
| 10 | `system/modules/robot/core/Execution/MismatchEscalationCoordinator*.cs` | 3275 / 103 | Mismatch/fail-closed escalation and release; pending-IEA deferral now split to a small partial. | First move-only split started; behavior simplification still requires fresh runtime proof. |
...
9. Next: checkpoint the pending-IEA coordinator split, then continue `MismatchEscalationCoordinator` move-only decomposition by responsibility before any authority behavior simplification.
