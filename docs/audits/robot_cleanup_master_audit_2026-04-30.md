...
| 10 | `system/modules/robot/core/Execution/MismatchEscalationCoordinator*.cs` | 2908 / 292 / 103 / 87 | Mismatch/fail-closed escalation and release; convergence suppression, pending-IEA deferral, and telemetry counters now split to small partials. | Move-only split in progress; behavior simplification still requires fresh runtime proof. |
...
9. Next: checkpoint the telemetry coordinator split, then continue `MismatchEscalationCoordinator` move-only decomposition by responsibility before any authority behavior simplification.
