#!/usr/bin/env python3
"""
Check recent logging status for tick and heartbeat events (corrected)
"""
import json
import glob
...
def main():
    print("=" * 80)
    print("RECENT LOGGING STATUS CHECK (CORRECTED)")
    print("=" * 80)
    print()
    ...
    if engine_events:
        ...
        for event in engine_events[-5:]:
            ts_utc = event.get('ts_utc', 'N/A')[:19] if event.get('ts_utc') else 'N/A'
            event_name = event.get('event', 'N/A')
            message = event.get('message', 'N/A')
            print(f"    {ts_utc} | {event_name} | {message[:60]}")
    else:
    ...
    for event_name, count in sorted(event_types.items(), key=lambda x: x[1], reverse=True)[:15]:
    ...
    if heartbeat_events:
        ...
        for event in heartbeat_events:
            event_name = event.get('event', 'UNKNOWN')
            heartbeat_types[event_name] += 1
        
        print("  By type:")
        for event_name, count in sorted(heartbeat_types.items(), key=lambda x: x[1], reverse=True):
            print(f"    {event_name}: {count}")
        ...
        for event in heartbeat_events[-10:]:
            ...
            print(f"    {ts_utc} | {event_name} | {stream}")
    else:
    ...
    for event in stream_tick_events:
        ...
        if 'TICK' in event_name and stream and stream != '__engine__':
            stream_tick_types[event_name] += 1
    
    if stream_tick_types:
        print("  Stream-level TICK events:")
        for event_name, count in sorted(stream_tick_types.items(), key=lambda x: x[1], reverse=True):
            print(f"    {event_name}: {count}")
    else:
