...
for stream_id in all_streams:
    ...
    # Find latest state from any event that has a 'state' field
...
# Separate ARMED from others
armed_streams = {k: v for k, v in stream_states.items() if v['state'] == 'ARMED'}
other_states = {k: v for k, v in stream_states.items() if v['state'] != 'ARMED'}

print(f"\nARMED STREAMS ({len(armed_streams)}):")
if armed_streams:
    for stream_id, info in sorted(armed_streams.items()):
        status = "COMMITTED" if info['committed'] else "ACTIVE"
        print(f"  {stream_id}: {status} (since {info['time']})")
else:
    print("  None")
...
# Get details for ARMED streams
if armed_streams:
    print(f"\n{'='*80}")
    print("ARMED STREAMS DETAILS")
    print("="*80)
    
    for stream_id in sorted(armed_streams.keys()):
        stream_events = [e for e in today_events if e.get('stream') == stream_id]
