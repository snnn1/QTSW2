#!/usr/bin/env python3
"""Check which streams are ARMED - detailed version"""
import json
...
for stream_id in all_streams:
    ...
    # Get latest state from events - check both 'state' field and event type
    state_events = [e for e in stream_events if e.get('state')]
    
    # Also check transition events
    transitions = [e for e in stream_events if 'TRANSITION' in e.get('event', '')]
    
    if not state_events and not transitions:
        continue
    ...
    # Check transition events for state info
    if transitions:
