#!/usr/bin/env python3
"""
Regenerate timetable **artifacts** from the existing master matrix (non-live).

Writes timestamped parquet/JSON under data/_scratch/timetable_regen — not timetable_current.json.
"""
...
def main():
    ...
    if master_df.empty:
        logger.error("No master matrix found.")
        return 1
    ...
    try:
        ...
        logger.warning("Stream filters: %s", e)
    ...
    if df.empty:
        logger.error("Empty timetable dataframe.")
        return 1
    pq, js = engine.save_timetable(df, str(out_dir))
    logger.info("Wrote %s and %s", pq, js)
    return 0
