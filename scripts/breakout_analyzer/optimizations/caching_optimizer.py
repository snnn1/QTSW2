...
class CachingOptimizer:
    ...
    def smart_year_filtering(self, file_paths: List[Union[str, Path]], 
        ...
        for file_path in file_paths:
            ...
            try:
                ...
                if file_years.intersection(target_years_set):
                    ...
                    print(f"✅ {file_path.name}: Contains years {sorted(file_years.intersection(target_years_set))}")
                else:
                    print(f"⏭️ {file_path.name}: Skipped (years: {sorted(file_years)})")
        ...
        print(f"📊 Smart filtering: {len(relevant_files)}/{len(file_paths)} files relevant")
    ...
    def _evict_oldest_cache_entry(self):
        ...
        print(f"🗑️ Evicted cache entry: {oldest_key[:8]}...")
    ...
    def preload_metadata(self, file_paths: List[Union[str, Path]]) -> Dict[str, FileMetadata]:
        ...
        """
        print(f"📊 Preloading metadata for {len(file_paths)} files...")
        ...
        with ThreadPoolExecutor(max_workers=min(4, len(file_paths))) as executor:
            ...
            for future in future_to_path:
                ...
                try:
                    ...
                    print(f"⚠️ Error reading metadata for {path}: {e}")
        
        print(f"✅ Metadata preloaded for {len(metadata_dict)} files")
