...
class ParallelProcessor:
    ...
    def process_ranges_parallel(self, ranges: List[Dict], process_func: Callable, 
        ...
        print(f"🚀 Starting parallel processing of {len(ranges)} ranges using {self.max_workers} threads")
        ...
        print(f"📊 Split into {len(range_chunks)} chunks (avg {chunk_size} ranges per chunk)")
    ...
    def _process_chunk(self, chunk: List[Dict], process_func: Callable, 
        ...
        for range_data in chunk:
            try:
                ...
                print(f"⚠️ Range processing failed: {exc}")
    ...
    def _process_ranges_sequential(self, ranges: List[Dict], process_func: Callable, 
        ...
        Returns:
            List of results from processing each range
        """
        print(f"🔄 Processing {len(ranges)} ranges sequentially")
    ...
    def process_dataframe_parallel(self, df: pd.DataFrame, ranges: List[Dict], 
        ...
        print(f"🚀 Starting parallel DataFrame processing of {len(ranges)} ranges")
        ...
        processing_time = time.time() - start_time
        print(f"✅ Parallel DataFrame processing completed in {processing_time:.2f} seconds")
    ...
    def _process_dataframe_sequential(self, df: pd.DataFrame, ranges: List[Dict], 
        ...
        Returns:
            List of results from processing each range
        """
        print(f"🔄 Processing DataFrame with {len(ranges)} ranges sequentially")
        ...
        processing_time = time.time() - start_time
        print(f"✅ Sequential DataFrame processing completed in {processing_time:.2f} seconds")
