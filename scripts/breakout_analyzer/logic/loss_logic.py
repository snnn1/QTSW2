...
class LossManager:
    ...
    def calculate_initial_stop_loss(self, entry_price: float, direction: str, 
        ...
        Args:
            ...
            range_size: Range size for stop loss calculation (legacy parameter)
            range_high: Range high price (preferred)
            range_low: Range low price (preferred)
            
        Returns:
            Initial stop loss price (capped at 3× target maximum)
        """
        # Maximum stop loss distance is 3× target
        max_sl_distance = self.config.initial_multiplier * target_pts  # 3× target
        
        # Use range boundaries if provided (preferred method)
        if range_high is not None and range_low is not None:
            if direction == "Long":
                range_based_sl = range_low  # For long trades, stop at range low
                sl_distance = entry_price - range_based_sl
            else:
                range_based_sl = range_high  # For short trades, stop at range high
                sl_distance = range_based_sl - entry_price
            
            # Cap stop loss distance at 3× target
            if sl_distance > max_sl_distance:
                # Stop loss too far - use capped distance instead
                if direction == "Long":
                    return entry_price - max_sl_distance
                else:
                    return entry_price + max_sl_distance
            else:
                # Range-based stop is within limit
                return range_based_sl
        
        # Fallback to old logic if range boundaries not provided
        if range_size is None:
            # Fallback to 3x target if no range size provided
            sl_distance = self.config.initial_multiplier * target_pts
        else:
            # Simple range-based stop loss logic
            if range_size <= 3 * target_pts:
                # If range ≤ 3× target: stop loss = range size
                sl_distance = range_size
            else:
                # If range > 3× target: stop loss = 3× target (but never more than range)
                sl_distance = min(3 * target_pts, range_size)
