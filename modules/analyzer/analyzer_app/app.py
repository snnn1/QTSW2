...
try:
    ...
    # Try to import UI performance optimizer (optional, doesn't affect core functionality)
    from ui_performance_monitor import UIPerformanceOptimizer, PerformanceMetrics
...
# -------------------------------------------------------------------
def list_parquet_files(base_dir="data/translated"):
    """
    Find parquet files in translated directory structure.
    Looks for files organized as: instrument/1m/year/month/*.parquet
    Returns combined files if found, otherwise individual date files.
    """
    base = Path(base_dir)
    if not base.exists():
        # Fallback to data/processed if translated doesn't exist
        base = Path("data/processed")
        if not base.exists():
            return []
        return sorted(base.glob("*.parquet"))
    
    # Check for combined files first (e.g., ES_2006-2025.parquet in root)
    combined_files = sorted(base.glob("*_*.parquet"))
    if combined_files:
        return combined_files
    
    # Otherwise, collect all parquet files from nested structure
    # Group by instrument and create virtual combined files
    all_files = sorted(base.glob("**/*.parquet"))
    
    # Group by instrument (extract from path like ES/1m/2025/12/ES_1m_2025-12-11.parquet)
    instrument_files = {}
    for file in all_files:
        # Extract instrument from path (first directory level)
        parts = file.parts
        if len(parts) >= 2:
            instrument = parts[-4] if len(parts) >= 4 else parts[0]  # ES/1m/year/month/file.parquet
            if instrument not in instrument_files:
                instrument_files[instrument] = []
            instrument_files[instrument].append(file)
    
    # Return all files (will be combined when selected)
    return all_files
...
def main():
    ...
    # UI performance optimizer (optional, doesn't affect core functionality)
    # All analyzer semantics live in run_strategy() - this is just for UI progress tracking
    ...
    # Try data/translated first, then fallback to data/processed
    parquet_files = list_parquet_files("data/translated")
    data_source = "data/translated"
    if not parquet_files:
        parquet_files = list_parquet_files("data/processed")
        data_source = "data/processed"
    
    if not parquet_files:
        st.error("No Parquet files found in data/translated/ or data/processed/. Run your data translator first.")
        return
    
    # If files are in nested structure, group by instrument
    if any("/" in str(f) or "\\" in str(f) for f in parquet_files):
        # Files are in nested structure - group by instrument
        instrument_groups = {}
        for f in parquet_files:
            # Extract instrument from path (e.g., data/translated/ES/1m/2025/12/file.parquet)
            parts = Path(f).parts
            # Find instrument directory (should be first level after data/translated)
            instrument = None
            for i, part in enumerate(parts):
                if part in ["ES", "NQ", "YM", "CL", "NG", "GC", "MES", "MNQ", "MYM", "MCL", "MNG", "MGC"]:
                    instrument = part
                    break
            
            if instrument is None:
                # Try to extract from filename
                instrument = Path(f).stem.split('_')[0].upper()
            
            if instrument not in instrument_groups:
                instrument_groups[instrument] = []
            instrument_groups[instrument].append(f)
        
        # Create selectbox for instruments
        instruments = sorted(instrument_groups.keys())
        selected_instrument = st.selectbox("Choose instrument:", instruments)
        
        # Show file count for selected instrument
        file_count = len(instrument_groups[selected_instrument])
        st.info(f"Found {file_count} parquet file(s) for {selected_instrument}. Files will be combined automatically.")
        
        # Store selected files for later loading
        selected_files = instrument_groups[selected_instrument]
        in_file = None  # Will load multiple files
    else:
        # Flat structure - single file selection
        file_options = [f.name for f in parquet_files]
        choice = st.selectbox("Choose input parquet file:", file_options)
        in_file = parquet_files[file_options.index(choice)]
        selected_files = [in_file]
    
    # Extract instrument from filename if not already set
    if 'selected_instrument' not in locals():
        if in_file:
            selected_instrument = in_file.stem.split('_')[0].upper()
        else:
            selected_instrument = selected_files[0].stem.split('_')[0].upper() if selected_files else "UNKNOWN"
    ...
    try:
        # Load data (handle both single file and multiple files)
        if in_file is None and 'selected_files' in locals() and selected_files:
            ...
            if dfs:
                ...
                df_temp = pd.DataFrame()  # Empty dataframe to prevent errors
        else:
        ...
        if 'timestamp' in df_temp.columns:
            ...
            st.warning("No timestamp column found in data. Year filtering not available.")
            selected_years = []
    except Exception as e:
    ...
    if inspect_data:
        ...
        # Auto-run when checkbox is enabled
        if True:
            # Load data (handle both single file and multiple files)
            if in_file is None and 'selected_files' in locals() and selected_files:
                # Load and combine all files for inspection
                dfs = []
                for file_path in selected_files:
                    try:
                        dfs.append(pd.read_parquet(file_path))
                        ...
                        st.warning(f"Error loading {file_path.name}: {e}")
                if dfs:
                    df_inspect = pd.concat(dfs, ignore_index=True)
                else:
                    st.error("No files could be loaded for inspection.")
                    return
            else:
    ...
    if validate_data:
        ...
        if True:
            ...
            if in_file is None and 'selected_files' in locals() and selected_files:
                # Load and combine all files for validation
                dfs = []
                for file_path in selected_files:
                    try:
                        dfs.append(pd.read_parquet(file_path))
                ...
                if dfs:
                    ...
                    st.error("No files could be loaded for validation.")
                    return
            else:
    ...
    if st.button("Run Analyzer", type="primary", use_container_width=True):
        ...
        # Handle both single file and multiple files (from nested structure)
        if in_file is None and 'selected_files' in locals() and selected_files:
            # Load and combine multiple files
            st.info(f"Loading and combining {len(selected_files)} file(s)...")
            dfs = []
            for i, file_path in enumerate(selected_files, 1):
                try:
                    ...
                    if i % 10 == 0 or i == len(selected_files):
                        st.info(f"Loaded {i}/{len(selected_files)} files...")
                except Exception as e:
                    st.warning(f"Error loading {file_path.name}: {e}")
                    continue
            ...
            st.success(f"Combined {len(dfs)} file(s) into {len(df):,} rows")
        else:
            # Load single file
        ...
        for inst in instruments:
            ...
            if debug_mode:
                ...
                with redirect_stdout(debug_output):
                    # All analyzer semantics live in run_strategy()
                ...
                if debug_text:
                    ...
                    print(f"No debug output generated for {inst}")
            ...
            if not res.empty:
                ...
                print(f"{inst} analysis completed: No trades generated")
        ...
        print(f"[RESULTS] Total results: {len(final)} trades")
