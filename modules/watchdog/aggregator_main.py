...
from .config import (
    ...
    WATCHDOG_CPU_DIAG_INTERVAL_SECONDS,
...
def _watchdog_info_for_stream_today(
    ...
    """Watchdog ``StreamStateInfo`` for **today's** session key only: ``(current_trading_date, stream_id)``."""
    return stream_states_dict.get((current_trading_date, stream_id))


def _latest_prior_watchdog_info_for_stream(
    stream_states_dict: Dict[Tuple[str, str], Any],
    current_trading_date: str,
    stream_id: str,
) -> Any:
    """
    Latest ``StreamStateInfo`` for ``stream_id`` among **strictly prior** session days only
    (``trading_date < current_trading_date``), lexicographic max on that subset.

    Excludes same-day and future-dated keys (e.g. mis-keyed rollover state) so carry-over labels
    stay consistent. Used for ``STREAM_FEED_PRIOR_ROW_DETECTED`` diagnostics only — **not** merged into feed rows.
    """
    current_td = current_trading_date
    prior_candidates = [
        (td, info)
        for (td, sid), info in stream_states_dict.items()
        if sid == stream_id and td < current_td
    ]
...
def _canonical_from_stream_id(stream_id: str) -> str:
    """Derive canonical instrument from stream id when timetable metadata is missing (e.g. ES1 -> ES, RTY2 -> RTY)."""
    if not stream_id:
        return ""
    sid = str(stream_id).strip().upper()
    while len(sid) > 1 and sid[-1].isdigit():
        sid = sid[:-1]
    return sid or str(stream_id).strip().upper()


def _session_from_stream_id(stream_id: str) -> str:
    """Session S1/S2 from trailing stream suffix."""
...
class WatchdogAggregator:
    ...
    def __init__(self):
        ...
        self._last_feed_ingest_diag: Dict[str, Any] = {}
    ...
    def _maybe_emit_cpu_diag(
        self,
        ms_raw_robot_phase: float,
        ms_feed_tail_phase: float,
        feed_sync_ran: bool,
        raw_events_returned: int,
    ) -> None:
        """
        Rate-limited INFO log when WATCHDOG_CPU_DIAG=1: where wall time went this cycle
        (raw robot logs merge/write vs frontend_feed tail parse/process). Helps correlate
        high CPU with order volume vs ENGINE_TICK_* diagnostic spam.
        """
        if not WATCHDOG_CPU_DIAG_ENABLED:
            return
        now = datetime.now(timezone.utc)
        if self._last_cpu_diag_utc is not None:
            if (now - self._last_cpu_diag_utc).total_seconds() < WATCHDOG_CPU_DIAG_INTERVAL_SECONDS:
                return
        self._last_cpu_diag_utc = now

        feed_metrics = getattr(self._event_feed, "last_cycle_metrics", None) or {}
        ingest = getattr(self, "_last_feed_ingest_diag", None) or {}
        ...
        payload: Dict[str, Any] = {
            "event_type": "WATCHDOG_CPU_DIAG",
            "timestamp_utc": now.isoformat(),
            "phase_ms_raw_robot_logs": round(ms_raw_robot_phase, 2),
            "phase_ms_feed_tail_ingest": round(ms_feed_tail_phase, 2),
            "feed_tail_ran": feed_sync_ran,
            "raw_log_cycle_returned_count": raw_events_returned,
            "frontend_feed_size_mb": feed_size_mb,
            "event_feed_last_cycle": feed_metrics,
            "feed_tail_last_cycle": ingest,
        }
        try:
            import os
            ...
            # Short block for a meaningful sample (first call is otherwise often 0.0)
            payload["watchdog_process_cpu_percent"] = round(p.cpu_percent(interval=0.05), 2)
            payload["watchdog_rss_mb"] = round(p.memory_info().rss / (1024 * 1024), 2)
        except Exception:
            payload["watchdog_process_cpu_percent"] = None
            payload["watchdog_rss_mb"] = None

        logger.info("WATCHDOG_CPU_DIAG: %s", json.dumps(payload, default=str))
    ...
    async def _process_events_loop(self):
        ...
        while self._running:
            try:
                ...
                ms_raw = (time.perf_counter() - t_raw) * 1000
                ...
                if FRONTEND_FEED_FILE.exists():
                    ...
                    self._last_feed_ingest_diag = {"tail_skipped_no_frontend_feed_file": True}

                self._maybe_emit_cpu_diag(ms_raw, ms_feed, feed_ran, int(processed_count or 0))
    ...
    def _process_feed_events_sync(self):
        ...
        try:
            ...
            if WATCHDOG_CPU_DIAG_ENABLED:
                tc = Counter(e.get("event_type", "") for e in parsed_events)
                ingest_diag["tail_top_event_types"] = dict(tc.most_common(15))
                ingest_diag["tail_order_related_count"] = sum(tc.get(t, 0) for t in ORDER_RELATED_EVENT_TYPES)
            self._last_feed_ingest_diag = ingest_diag
    ...
    def _ensure_flatten_baselines_for_timetable_streams(
        self,
        stream_order: List[str],
        timetable_streams_metadata: Dict[str, Any],
        current_trading_date: str,
        stream_states_dict: Dict[Tuple[str, str], Any],
    ) -> None:
        """Pre-create NOT_TRIGGERED tracker rows for every timetable stream (matches stream_session_flatten_fields keys)."""
        ...
        for stream_id in stream_order:
            ...
            instrument = (timetable_meta.get("instrument", "") or "").strip() or _canonical_from_stream_id(
                stream_id
            )
            session = (timetable_meta.get("session", "") or "").strip() or _session_from_stream_id(stream_id)
            session_class_meta = (timetable_meta.get("session_class") or "").strip()
            watchdog_info = _watchdog_info_for_stream_today(
                stream_states_dict, current_trading_date, stream_id
            )
            if watchdog_info:
                instrument_for_flatten = (
                    (getattr(watchdog_info, "instrument", None) or instrument or "").strip()
                )
                session_for_flatten = (
                    (getattr(watchdog_info, "session", None) or session or "").strip()
                )
            else:
                instrument_for_flatten = (instrument or "").strip()
                session_for_flatten = (session or "").strip()
            tracker.ensure_baseline_row(
                trading_date=current_trading_date,
                session=session_for_flatten,
                session_class=session_class_meta or None,
                instrument=instrument_for_flatten,
                stream=stream_id,
            )

    def get_session_flatten_state(self) -> List[Dict[str, Any]]:
        """Session close + flatten lifecycle rows (engine events + timetable baselines)."""
        tracker = getattr(self, "_session_flatten_tracker", None)
        if tracker is None:
            return []
        try:
            ...
            if not current_trading_date:
                chicago_now = datetime.now(CHICAGO_TZ)
                current_trading_date = compute_timetable_trading_date(chicago_now)
            enabled = self._state_manager.get_enabled_streams()
            if enabled is not None:
                meta = self._state_manager.get_timetable_streams_metadata() or {}
                stream_order = [
                    sid
                    for sid in self._state_manager.get_enabled_streams_ordered()
                    if sid in enabled
                ]
                if len(stream_order) != len(enabled):
                    for sid in sorted(enabled - set(stream_order)):
                        stream_order.append(sid)
                if enabled and not stream_order:
                    stream_order = sorted(enabled)
                stream_states_dict = self._state_manager.get_stream_states()
                self._ensure_flatten_baselines_for_timetable_streams(
                    stream_order, meta, current_trading_date, stream_states_dict
                )
        except Exception as e:
    ...
    def get_stream_states(self) -> Dict:
        ...
        Live robot state (``state``, ranges, etc.) is merged **only** for ``(current_trading_date, stream_id)``
        from ``WatchdogStateManager._stream_states``. Execution feed rows are **only**
        ``trading_date == current_trading_date``; prior-session watchdog keys are not surfaced as carry-over
        columns (diagnostic log only: ``STREAM_FEED_PRIOR_ROW_DETECTED``).
        ...
        try:
            ...
            if enabled_streams is not None:
                ...
                for stream_id in stream_order:
                    ...
                    prior_info = _latest_prior_watchdog_info_for_stream(
                        stream_states_dict, current_trading_date, stream_id
                    )
                    if watchdog_info is None and prior_info is not None:
                        logger.info(
                            "STREAM_FEED_PRIOR_ROW_DETECTED trading_date=%s stream=%s instrument=%s session=%s",
                            current_trading_date,
                            stream_id,
                            (timetable_meta.get("instrument", "") or "").strip() or _canonical_from_stream_id(
                                stream_id
                            ),
                            (timetable_meta.get("session", "") or "").strip() or _session_from_stream_id(
                                stream_id
                            ),
                        )
                    ...
                    flatten_fields = stream_session_flatten_fields(
                        getattr(self, "_session_flatten_tracker", None),
                        trading_date=current_trading_date,
                        session=session_for_flatten,
                        session_class=session_class_meta or None,
                        instrument=instrument_for_flatten,
                ...
                streams = [
                    s for s in streams if str(s.get("trading_date") or "").strip() == current_trading_date
                ]
                if len(streams) != _streams_before_td_filter:
                    logger.warning(
                        "STREAM_FEED_DATE_FILTER dropped rows: before=%s after=%s trading_date=%s",
                        _streams_before_td_filter,
                        len(streams),
                        current_trading_date,
                    )
