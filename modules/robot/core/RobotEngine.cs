...
public sealed class RobotEngine : IExecutionRecoveryGuard
{
    ...
    /// <summary>Gate reconciliation tracing: detect flat progress across expensive recovery cycles (instrument-scoped).</summary>
    private sealed class GateReconCycleProbeState
    {
        public bool HasPriorCycleEnd;
        public int LastPendingAdoption;
        public int LastUnexplainedWorking;
        public int LastIeaOwned;
        public int ConsecutiveZeroDeltaCycles;
    }

    private readonly Dictionary<string, GateReconCycleProbeState> _gateReconCycleProbeByInstrument =
        new(StringComparer.OrdinalIgnoreCase);
    ...
    public void OnBar(DateTimeOffset barUtc, string instrument, decimal open, decimal high, decimal low, decimal close, DateTimeOffset utcNow)
    {
        ...
        if (!_activeTradingDate.HasValue)
        {
            ...
            var nowChicago = _time.ConvertUtcToChicago(utcNow);
            var timetableTradingDate = string.IsNullOrWhiteSpace(_lastTimetable?.session_trading_date) ? "NOT_LOADED" : _lastTimetable!.session_trading_date.Trim();
        ...
        Exception? parseException)
    {
        ...
        // matches modules.timetable.cme_session.get_cme_trading_date — clamp in-memory contract so lock and
        // ApplyTimetable agree (avoids SESSION_START_DATE_MISMATCH stand-down + empty GetTradingDate()).
        ...
        else if (_activeTradingDate.Value != timetableTradingDate.Value)
        {
            if (timetableTradingDate.Value > _activeTradingDate.Value)
            {
                ...
                _streams.Clear();
                LogEvent(RobotEvents.EngineBase(utcNow, tradingDate: _activeTradingDate.Value.ToString("yyyy-MM-dd"), eventType: "TRADING_DATE_ROLLED", state: "ENGINE",
                    new
                    {
                        old_trading_date = previousLocked.ToString("yyyy-MM-dd"),
                        new_trading_date = _activeTradingDate.Value.ToString("yyyy-MM-dd"),
                        source = "engine",
                        streams_cleared = clearedStreams,
                        timetable_path = _timetablePath,
                        note = "Canonical roll event; timetable advanced to a later session day — streams cleared for rebuild (legacy event name: TRADING_DATE_ROLLED_FORWARD)"
                    }));
            }
            else
            {
    ...
    private void ApplyTimetable(TimetableContract timetable, DateTimeOffset utcNow, bool allowNewStreams = true)
    {
        ...
        var applyTimetableSessionRaw = (timetable.session_trading_date ?? "").Trim();
        if (string.IsNullOrWhiteSpace(applyTimetableSessionRaw))
        {
            LogEvent(RobotEvents.EngineBase(utcNow, tradingDate: _activeTradingDate.Value.ToString("yyyy-MM-dd"), eventType: "TIMETABLE_APPLY_SESSION_INVALID_CRIT", state: "ENGINE",
                new { reason = "Timetable missing session_trading_date on apply", note = "Standing down — not bypassable" }));
            StandDown();
            return;
        }
        if (!DateOnly.TryParse(applyTimetableSessionRaw, out var applyTtDate))
        {
            LogEvent(RobotEvents.EngineBase(utcNow, tradingDate: _activeTradingDate.Value.ToString("yyyy-MM-dd"), eventType: "TIMETABLE_APPLY_SESSION_INVALID_CRIT", state: "ENGINE",
                new { session_trading_date = applyTimetableSessionRaw, note = "Cannot parse timetable session date — standing down" }));
            StandDown();
            return;
        }
        if (applyTtDate != _activeTradingDate.Value)
        {
            LogEvent(RobotEvents.EngineBase(utcNow, tradingDate: _activeTradingDate.Value.ToString("yyyy-MM-dd"), eventType: "TIMETABLE_APPLY_SESSION_LOCK_MISMATCH_CRIT", state: "ENGINE",
                new
                {
                    locked_trading_date = _activeTradingDate.Value.ToString("yyyy-MM-dd"),
                    timetable_session_trading_date = applyTtDate.ToString("yyyy-MM-dd"),
                    note = "Timetable session date must match locked session — standing down"
                }));
            StandDown();
            return;
        }
        ...
        foreach (var directive in incoming)
        {
            ...
            else
            {
                ...
                TimetableStream.EnsureExecutionFields(modifiedDirective);
    ...
    private void HandleReconciliationQtyMismatchP2(string instrument, DateTimeOffset utcNow, string reason)
    {
        ...
        var journalQty = _executionJournal.GetOpenJournalQuantitySumForInstrumentFromMap(openJournalMap, inst, canonicalForJournal);
        ...
        int gateCycleOneBased)
    {
            ...
            ieaRecover != null)
        {
            try
            {
                adoptionScheduled = ieaRecover.TryScheduleRecoveryAdoptionScan(out adoptedInline);
            }
            catch
            {
        ...
        if (!_gateReconCycleProbeByInstrument.TryGetValue(inst, out var probe))
        {
            probe = new GateReconCycleProbeState();
            _gateReconCycleProbeByInstrument[inst] = probe;
        }

        var dp = readiness.DiagnosticPendingAdoptionCandidateCount - (probe.HasPriorCycleEnd ? probe.LastPendingAdoption : 0);
        var du = readiness.UnexplainedBrokerWorkingCount - (probe.HasPriorCycleEnd ? probe.LastUnexplainedWorking : 0);
        var di = readiness.DiagnosticIeaOwnedPlusAdoptedWorking - (probe.HasPriorCycleEnd ? probe.LastIeaOwned : 0);
        if (!probe.HasPriorCycleEnd)
        {
            dp = 0;
            du = 0;
            di = 0;
        ...
        if (dp == 0 && du == 0 && di == 0 && probe.HasPriorCycleEnd)
            probe.ConsecutiveZeroDeltaCycles++;
        else
            probe.ConsecutiveZeroDeltaCycles = 0;

        probe.HasPriorCycleEnd = true;
        probe.LastPendingAdoption = readiness.DiagnosticPendingAdoptionCandidateCount;
        probe.LastUnexplainedWorking = readiness.UnexplainedBrokerWorkingCount;
        probe.LastIeaOwned = readiness.DiagnosticIeaOwnedPlusAdoptedWorking;
        ...
        LogEvent(RobotEvents.EngineBase(utcNow, "", "RECONCILIATION_CYCLE_STATE", "ENGINE",
            new
            {
                ...
                delta_pending_adoption = dp,
                delta_unexplained_working = du,
                delta_iea_owned = di,
                adoption_recovery_scheduled_or_active = adoptionScheduled,
                adoption_delta_if_inline = adoptedInline,
                consecutive_zero_delta_cycles = probe.ConsecutiveZeroDeltaCycles,
        ...
        if (probe.ConsecutiveZeroDeltaCycles >= 3)
        {
            LogEvent(RobotEvents.EngineBase(utcNow, "", "RECONCILIATION_NO_PROGRESS_CYCLE", "ENGINE",
                new
                {
                    instrument = inst,
                    gate_cycle = gateCycleOneBased,
                    ...
                    root_cause_class = rootCauseClass,
                    contradictions = string.Join(";", readiness.Contradictions ?? new List<string>()),
                    note = "≥3 consecutive expensive cycles with flat pending / unexplained / IEA-trusted deltas"
                }));
        }
    ...
    private ReconciliationForcedConvergenceResult RunForcedBrokerAlignment(ReconciliationForcedConvergenceContext ctx)
    {
        ...
        InstrumentExecutionAuthority? ieaForStale = null;
        if (useIea && ReconciliationIeaLookup.TryResolve(account, inst, CountBrokerWorkingOrders(snap, inst), GetExecutionInstrument, out var ieaSnap) &&
            ieaSnap != null)
            ieaForStale = ieaSnap;
        var canonicalForJournal = GetCanonicalInstrument(inst) ?? inst;
        var posAbs = SumBrokerPositionQty(snap, inst);
        var tagIds = CollectRobotTaggedIntentIdsForInstrument(snap, inst);
        _executionJournal.ReconcileStaleAdoptionJournalCandidatesForRelease(
            ieaForStale?.ExecutionInstrumentKey ?? inst,
            canonicalForJournal,
            posAbs,
            tagIds,
            utcNow);
