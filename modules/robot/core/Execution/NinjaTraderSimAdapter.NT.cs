...
public sealed partial class NinjaTraderSimAdapter
{
    ...
    private void HandleExecutionUpdateReal(object executionObj, object orderObj)
    {
        ...
        if (TrySkipDuplicateExecutionUpdate50ms(instTrace, execIdTrace, fillQtyTrace, ordStateDedup, utcNow, out var execDedupSkips))
        {
            _log.Write(RobotEvents.ExecutionBase(utcNow, intentId ?? "", instTrace, "EXECUTION_DEDUP_SKIPPED", new
            {
    ...
    private void CheckAllInstrumentsForFlatPositions(DateTimeOffset utcNow)
    {
        ...
        catch (Exception ex)
        {
            ...
            _log.Write(RobotEvents.EngineBase(utcNow, "", "", "CHECK_ALL_INSTRUMENTS_FLAT_ERROR",
                new
                {
                    error = ex.Message,
                    exception_type = ex.GetType().Name,
