...
public sealed class MismatchEscalationCoordinator
{
    ...
    private void ProcessCleanPass(string instrument, DateTimeOffset utcNow)
    {
        ...
        {
            var prevState = state.EscalationState;
            state.EscalationState = MismatchEscalationState.NONE;
            state.Blocked = false;
            state.BlockReason = "";
            state.ConsecutiveCleanPassCount = 0;
            _mismatchClearedCount++;
            var clearedPayload = ToPayload(state, instrument, utcNow);
            _log?.Write(RobotEvents.ExecutionBase(utcNow, "", instrument, "RECONCILIATION_MISMATCH_CLEARED", clearedPayload));
            EmitCanonical(instrument, ExecutionEventTypes.MISMATCH_CLEARED, utcNow, clearedPayload);
        }
    }
    ...
    private void ProcessObservation(MismatchObservation obs)
    {
        ...
        if (state.EscalationState == MismatchEscalationState.DETECTED)
        {
            if (state.PersistenceMs >= MismatchEscalationPolicy.MISMATCH_PERSISTENT_THRESHOLD_MS)
            {
                ...
                var persistentPayload = ToPayload(state, inst, utcNow, obs);
                _log?.Write(RobotEvents.ExecutionBase(utcNow, "", inst, "RECONCILIATION_MISMATCH_PERSISTENT", persistentPayload));
                _log?.Write(RobotEvents.ExecutionBase(utcNow, "", inst, "RECONCILIATION_MISMATCH_BLOCKED", persistentPayload));
                EmitCanonical(inst, ExecutionEventTypes.MISMATCH_PERSISTENT, utcNow, persistentPayload, "WARN");
                EmitCanonical(inst, ExecutionEventTypes.MISMATCH_BLOCKED, utcNow, persistentPayload, "ERROR");
            }
            return;
        }
