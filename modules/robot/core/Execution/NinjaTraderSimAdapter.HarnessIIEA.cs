...
public sealed partial class NinjaTraderSimAdapter
{
    ...
    void INtActionExecutor.ExecuteCancelOrders(NtCancelOrdersCommand cmd) =>
        ThrowNtActionHarness("ExecuteCancelOrders");

    void INtActionExecutor.ExecuteFlattenInstrument(NtFlattenInstrumentCommand cmd) =>
        ThrowNtActionHarness("ExecuteFlattenInstrument");

    void INtActionExecutor.ExecuteSubmitEntryIntent(NtSubmitEntryIntentCommand cmd) =>
        ThrowNtActionHarness("ExecuteSubmitEntryIntent");

    void INtActionExecutor.ExecuteSubmitMarketReentry(NtSubmitMarketReentryCommand cmd) =>
        ThrowNtActionHarness("ExecuteSubmitMarketReentry");

    private static void ThrowNtActionHarness(string method) =>
        throw new NotImplementedException($"NinjaTraderSimAdapter.{method}: strategy-thread NT actions are not available in harness build (compile NinjaTraderSimAdapter.NT.cs with NINJATRADER).");
}
