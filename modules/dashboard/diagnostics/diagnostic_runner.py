...
class DiagnosticRunner:
    ...
    def test_export_timeout_logic(self) -> tuple:
        ...
        try:
            # Check if timeout logic exists in orchestrator or frontend
            # OLD: daily_data_pipeline_scheduler.py (removed)
            orchestrator_runner = QTSW2_ROOT / "dashboard" / "backend" / "orchestrator" / "runner.py"
            frontend_hook = QTSW2_ROOT / "dashboard" / "frontend" / "src" / "hooks" / "usePipelineState.js"
            
            checks = {}
            
            if orchestrator_runner.exists():
                content = orchestrator_runner.read_text()
                checks["orchestrator_has_timeout"] = "60" in content or "timeout" in content.lower()
    ...
    def test_stall_detection(self) -> tuple:
        ...
        try:
            ...
            if orchestrator_runner.exists():
                content = orchestrator_runner.read_text()
                checks["orchestrator_has_stall_detection"] = "5" in content or "stall" in content.lower() or "progress" in content.lower()
            
            if frontend_hook.exists():
                content = frontend_hook.read_text()
                checks["frontend_has_stall_detection"] = "5" in content or "stall" in content.lower() or "progress" in content.lower()
...
"""
        ...
        md += "- `dashboard/backend/orchestrator/` - Pipeline orchestrator (replaces daily_data_pipeline_scheduler.py)\n"
