...
A complete Pipeline Orchestrator subsystem has been implemented in `modules/orchestrator/`. This provides a single authority for pipeline runs, state management, scheduling, and event emission.
...
✅ **Completed:**
- All routers migrated to orchestrator
- WebSocket uses EventBus (no file tailing)
- Scheduler integrated
- Watchdog integrated
- Phase-1 always-on simplification complete
- JSONL monitor removed (events flow directly: Pipeline → EventBus → WebSocket → UI)
