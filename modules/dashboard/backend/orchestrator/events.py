...
class EventBus:
    ...
    async def publish(self, event: Dict):
        ...
        if event_key not in self.LIVE_EVENT_TYPES:
    ...
    def _write_to_jsonl_file(self, event: Dict, run_id: str, stage: str, event_type: str):
        ...
        """
        # Note: EventBus filtering happens above, this always writes if it's a loggable event
        ...
        # Special handling for 'log' events - only keep for pipeline/scheduler stages
        if event_type == 'log' and stage not in self.ALWAYS_LOG_STAGES:
            should_log_to_file = False
        
        # Skip verbose events (metric, progress, heartbeat, file_start, file_finish)
        if not should_log_to_file and event_type in self.VERBOSE_EVENTS:
            should_log_to_file = False
        
        # Write to JSONL file (only if should_log_to_file)
