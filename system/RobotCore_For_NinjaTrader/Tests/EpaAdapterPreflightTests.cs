...
public static class EpaAdapterPreflightTests
{
    ...
    {
        if (ExecutionPermissionAuthority.TryAdapterOrderSubmitPreflight(() => true, null, null, (_, _) => false, "MES", PathEntry, out var d1) ||
            d1 != "GLOBAL_KILL_SWITCH_ACTIVE")
            return (false, "Expected deny GLOBAL_KILL_SWITCH_ACTIVE when kill callback true");

        if (ExecutionPermissionAuthority.TryAdapterOrderSubmitPreflight(() => false, _ => true, null, (_, _) => false, "MES", PathEntry, out var d2) ||
            d2 != "MISMATCH_EXECUTION_BLOCK")
            return (false, "Expected deny MISMATCH_EXECUTION_BLOCK when mismatch callback true");

        if (ExecutionPermissionAuthority.TryAdapterOrderSubmitPreflight(() => false, _ => false, null, (_, _) => true, "MES", PathEntry, out var d3) ||
            d3 != "INSTRUMENT_FROZEN_OR_EPA_BLOCKED")
            return (false, "Expected deny INSTRUMENT_FROZEN_OR_EPA_BLOCKED when frozen callback true");

        if (!ExecutionPermissionAuthority.TryAdapterOrderSubmitPreflight(null, null, null, null, "MES", PathEntry, out var d4) || d4 != "")
            return (false, "Expected allow when all instrument callbacks null");

        if (!ExecutionPermissionAuthority.TryAdapterOrderSubmitPreflight(() => false, _ => false, null, (_, _) => false, "MES", PathEntry, out var d5) ||
            d5 != "")
            return (false, "Expected allow when callbacks deny false");

        if (!ExecutionPermissionAuthority.TryAdapterOrderSubmitPreflight(() => false, null, null, (inst, _) => inst == "MES", "NQ", PathEntry, out var d6) ||
            d6 != "")
            return (false, "Expected allow when frozen callback false for this instrument");

        if (!ExecutionPermissionAuthority.TryAdapterOrderSubmitPreflight(() => false, _ => true, null, (_, _) => false, "MES", PathEntry, out var d7,
