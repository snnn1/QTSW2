...
public sealed class RangeLockedEventPersister
{
    ...
    private static readonly Dictionary<string, RangeLockedEventPersister> _instances = new(StringComparer.OrdinalIgnoreCase);
    ...
    {
        _rangesDir = RobotRunArtifactPaths.LogsRanges(projectRoot);
    ...
    public static RangeLockedEventPersister GetInstance(string projectRoot)
    {
        ...
        lock (_instanceLock)
        {
            ...
            {
                inst = new RangeLockedEventPersister(projectRoot);
                _instances[key] = inst;
            }
            return inst;
        }
