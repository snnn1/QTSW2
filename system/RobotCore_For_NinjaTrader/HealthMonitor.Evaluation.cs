...
public sealed partial class HealthMonitor
{
    ...
    public void UpdateEngineTick(DateTimeOffset tickUtc, DateTimeOffset observedUtc)
    {
        ...
        lock (_sharedEngineTickLock)
        {
            ...
            if (observedUtc > _sharedLastEngineTickObservedUtc)
                _sharedLastEngineTickObservedUtc = observedUtc;
        }
    ...
    private void EvaluateEngineTickStall(DateTimeOffset utcNow)
    {
        ...
        DateTimeOffset lastObservedTick;
        lock (_sharedEngineTickLock)
        {
            lastTick = _sharedLastEngineTickUtc;
            lastObservedTick = _sharedLastEngineTickObservedUtc;
        }
        ...
        var elapsedReferenceUtc = _playbackTelemetryMode && lastObservedTick != DateTimeOffset.MinValue
            ? lastObservedTick
            : lastTick;
        ...
        if (!_engineTickStallActive)
        {
            ...
            _log.Write(RobotEvents.EngineBase(utcNow, tradingDate: "", eventType: eventType, state: "ENGINE",
                new
                {
                    ...
                    last_tick_observed_utc = lastObservedTick == DateTimeOffset.MinValue ? null : lastObservedTick.ToString("o"),
