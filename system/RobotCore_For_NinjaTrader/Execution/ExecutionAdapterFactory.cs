...
public static class ExecutionAdapterFactory
{
    ...
    /// </summary>
    /// <param name="persistenceRoot">Run-scoped persistence root for SIM (execution journals, incidents, trace). When null, SIM uses <paramref name="repositoryRoot"/>.</param>
    /// <param name="time">TimeService for LIVE adapter (breakout validity gate). Optional; defaults to America/Chicago.</param>
    public static IExecutionAdapter Create(ExecutionMode mode, string repositoryRoot, RobotLogger log, ExecutionJournal? executionJournal = null, TimeService? time = null, string? persistenceRoot = null)
    {
        ...
        switch (mode)
        {
            ...
            {
                var stateRoot = string.IsNullOrWhiteSpace(persistenceRoot) ? repositoryRoot : persistenceRoot!;
                adapter = new NinjaTraderSimAdapter(repositoryRoot, stateRoot, log, executionJournal ?? new ExecutionJournal(stateRoot, log));
                log.Write(RobotEvents.EngineBase(
                    DateTimeOffset.UtcNow,
                    tradingDate: "",
                    eventType: "ADAPTER_SELECTED",
