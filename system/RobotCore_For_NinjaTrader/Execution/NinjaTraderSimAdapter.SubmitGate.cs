...
public sealed partial class NinjaTraderSimAdapter
{
        ...
        out ExecutionSafetySnapshot snap)
    {
        ...
        if (!ExecutionStructuralLayer.TryEvaluateFlattenStructure(req, out snap, out var flatReason))
        {
            ...
            {
                var cleanCorrelationId = correlationId?.Trim();
                if (cleanCorrelationId?.StartsWith("SESSION_CLOSE_FLATTEN:", StringComparison.OrdinalIgnoreCase) == true &&
                    snap.BrokerQty <= 0 &&
                    snap.JournalQty <= 0)
                {
                    TryAppendLateSessionCloseFlattenConfirmedKeyEvent(utcNow, inst, cleanCorrelationId, null, intentId ?? "");
                }
