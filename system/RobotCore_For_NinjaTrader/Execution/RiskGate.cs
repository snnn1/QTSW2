...
/// <summary>
/// Stream-attributed <b>risk-increasing</b> gate: fail-closed checks before bracket placement and other
/// stream-driven entries (see docs/authority/02_decision_authority.md §2, §5, §9).
/// Not all broker operations use this type — re-entry MARKET, post-fill protectives, flatten, cancel, and BE
/// use other EPA stacks.
/// 
/// RiskGate enforces blocking during recovery states for paths that call <see cref="CheckGates"/>.
/// Risk-reducing operations (flatten family) bypass this gate by design.
/// 
/// During DISCONNECT_FAIL_CLOSED and RECONNECTED_RECOVERY_PENDING:
/// - Stream RI submits that call CheckGates() → blocked (via IExecutionRecoveryGuard)
/// - Flatten / emergency flatten → permitted (bypasses RiskGate)
/// G5: Kill switch is evaluated before recovery in <see cref="CheckGates"/> so the single surfaced denial reason
/// follows precedence GLOBAL_KILL_SWITCH over RECOVERY when both would block.
/// </summary>
public sealed class RiskGate
{
    ...
    /// <summary>G1: EPA/engine-owned mismatch execution block (durable decision path); checked before generic frozen/supervisory.</summary>
    private readonly Func<string, bool>? _isMismatchExecutionBlocked;
        ...
        DateTimeOffset utcNow)
    {
        ...
        // Gate -1a: G1 — mismatch execution block (EPA/engine authority; not parallel INSTRUMENT_FROZEN)
        if (_isMismatchExecutionBlocked != null && _isMismatchExecutionBlocked(instrument))
        {
            failedGates.Add("MISMATCH_EXECUTION_BLOCK");
        ...
        // Gate 0: Kill switch (before recovery — G5 surfaced denial precedence when both apply)
        ...
        // Gate 1: G3 — engine recovery guard only (IExecutionRecoveryGuard); no independent recovery inference
