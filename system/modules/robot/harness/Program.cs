...
if (testIndex >= 0 && testIndex + 1 < argsList.Count)
{
    ...
    else if (testName.Equals("STREAM_JOURNAL_PLAYBACK_BYPASS", StringComparison.OrdinalIgnoreCase))
    {
    ...
    else if (testName.Equals("SCENARIO_HARNESS_SIX", StringComparison.OrdinalIgnoreCase))
    {
...
static void RunScenarioHarnessSix()
{
    Console.WriteLine("=== Scenario Harness (Six) — replay + rebuild validation ===");
    Console.WriteLine("  Scenarios 1–6: normal entry, deferred execution, dedup, stop-cancel recovery, stop-cancel flatten, persistent mismatch");
    Console.WriteLine();

    var (allPassed, metrics, results) = ExecutionScenarioRunner.RunFirstSix(msg => Console.WriteLine(msg));
    ...
    if (!allPassed)
    {
        Console.WriteLine("SCENARIO_HARNESS_SIX FAILED");
        Environment.Exit(1);
    }

    Console.WriteLine("ALL SIX SCENARIOS PASSED");
    Environment.Exit(0);
}
