...
public sealed class ExecutionEventWriter
{
    ...
    private readonly ExecutionEventSequence _sequence = new();
    private readonly object _writeLock = new object();

    private int _writeCount;
    private int _writeFailures;
    private readonly HashSet<string> _streamsOpened = new(StringComparer.OrdinalIgnoreCase);

    /// <param name="persistenceRoot">Run root for <c>events/execution_events/&lt;trading_date&gt;/</c> (engine <c>_persistenceBase</c>).</param>
    public ExecutionEventWriter(string persistenceRoot, Func<string> getTradingDate, RobotLogger? log, Func<string>? getRunId = null)
    {
        _projectRoot = persistenceRoot ?? throw new ArgumentNullException(nameof(persistenceRoot));
    ...
    private string GetStreamPath(string tradingDate, string instrument)
    {
        ...
        var baseDir = RobotRunArtifactPaths.EventsExecutionEventsTradingDate(_projectRoot, td);
