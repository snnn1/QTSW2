...
public static class UnmatchedPositionPolicyEvaluator
{
        ...
        RobotLogger? log)
    {
        ...
        foreach (var o in workingOrders)
        {
            ...
            if (!IsSameInstrument(o.Instrument, instrument) &&
                !IsSameInstrument(o.Instrument, execVariant))
                continue;
    ...
    private static bool IsSameInstrument(string? a, string? b)
    {
        if (string.IsNullOrEmpty(a) || string.IsNullOrEmpty(b)) return false;
        var aNorm = a.Trim().ToUpperInvariant();
        var bNorm = b.Trim().ToUpperInvariant();
        if (aNorm == bNorm) return true;
        var aCanon = GetCanonicalForPositionInstrument(aNorm);
        var bCanon = GetCanonicalForPositionInstrument(bNorm);
