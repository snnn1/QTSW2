...
public static class ExecutionScenarioRunner
{
    ...
    /// <summary>
    /// Run the first six canonical scenarios (<see cref="ExecutionScenarioDefinitions.GetFirstSix"/>). Aborts on first failure.
    /// </summary>
    public static (bool AllPassed, ScenarioMetrics Metrics, List<ScenarioResult> Results) RunFirstSix(
        Action<string>? log = null)
    {
        return RunScenarios(ExecutionScenarioDefinitions.GetFirstSix(), log);
    }
        ...
        Action<string>? log)
    {
        ...
        try
        {
            ...
            var eventDir = RobotRunArtifactPaths.EventsExecutionEventsTradingDate(tempBase, tradingDate);
            ...
            for (var i = 0; i < scenarios.Count; i++)
            {
                ...
                var scenarioEventDir = RobotRunArtifactPaths.EventsExecutionEventsTradingDate(scenarioDir, tradingDate);
