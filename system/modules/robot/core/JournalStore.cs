...
public sealed class JournalStore
{
    ...
    {
        _journalDir = RobotRunArtifactPaths.StateStreamJournals(projectRoot);
    ...
    /// <returns>True if the journal was written to disk successfully; false if persistence failed after retry.</returns>
    public bool Save(StreamJournal journal)
    {
        string path;
        string json;
        try
        {
            path = GetJournalPath(journal.TradingDate, journal.Stream);
            json = JsonUtil.Serialize(journal);
        }
        catch
        {
            return false;
        ...
        lock (fileLock)
        {
            if (TryAtomicWrite(path, json))
                return true;
            Thread.Sleep(10);
            return TryAtomicWrite(path, json);
        }
    }
