...
public sealed class RangeBuildingSnapshotPersister
{
    ...
    private static readonly Dictionary<string, RangeBuildingSnapshotPersister> _instances = new(StringComparer.OrdinalIgnoreCase);
    ...
    private RangeBuildingSnapshotPersister(string projectRoot)
    {
        ...
        _logDir = RobotRunArtifactPaths.LogsRangeBuilding(projectRoot ?? "");
    ...
    public static RangeBuildingSnapshotPersister GetInstance(string projectRoot)
    {
        ...
        lock (_instanceLock)
        {
            ...
            {
                inst = new RangeBuildingSnapshotPersister(projectRoot);
                _instances[key] = inst;
            }
            return inst;
        }
    ...
    public RestoreDiagnostics GetRestoreDiagnostics(string tradingDay, string streamId)
    {
        ...
        if (fileExists)
        {
            try
            {
                ...
                {
                    if (!string.IsNullOrWhiteSpace(line) && line.Contains(streamMarker, StringComparison.Ordinal))
                        streamLineCount++;
                }
            }
            catch { /* best-effort */ }
