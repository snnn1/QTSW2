...
/// Singleton pattern: one service per persistence root (engine <c>_persistenceBase</c> for run-scoped notification logs).
...
public sealed class NotificationService : IDisposable
{
    ...
    private void WriteNotificationLog(string level, string message)
    {
        try
        {
            var logDir = RobotRunArtifactPaths.LogsRobot(_projectRoot);
            Directory.CreateDirectory(logDir);
            var logPath = Path.Combine(logDir, "notification_errors.log");
