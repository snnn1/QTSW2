...
public sealed class ExecutionTraceWriter
{
    ...
    /// <summary>Creates writer for logs/robot/execution_trace.jsonl under the persistence root; returns null if path invalid.</summary>
    /// <remarks>Off by default; set env <c>QTSW2_EXECUTION_TRACE=1</c> to enable file I/O.</remarks>
    /// <param name="persistenceRoot">State root (e.g. engine <c>_persistenceBase</c>, including isolated playback).</param>
    public static ExecutionTraceWriter? TryCreate(string persistenceRoot)
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("QTSW2_EXECUTION_TRACE"), "1", StringComparison.Ordinal))
            return null;
        if (string.IsNullOrWhiteSpace(persistenceRoot)) return null;
        try
        {
            var dir = RobotRunArtifactPaths.LogsRobot(persistenceRoot.Trim());
