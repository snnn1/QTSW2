...
public sealed class RobotLogger
{
    ...
    /// <summary>Absolute path to the active <c>logs/robot</c> directory (matches <see cref="RobotLoggingService"/> when used).</summary>
    private string _robotLogDir;
        ...
        bool deferPhysicalInitUntilRebind = false)
    {
        ...
        RecalculateJsonlPath();
    }

    /// <summary>After <see cref="RobotEngine"/> rebases persistence (e.g. isolated playback), re-point async logging + fallback paths.</summary>
    public void RebindLogging(RobotLoggingService? loggingService, string absoluteRobotLogDir)
    {
        _loggingService = loggingService;
        ...
        {
            // best-effort; Write() paths also create as needed
        }
        RecalculateJsonlPath();
    }

    private void RecalculateJsonlPath()
    {
        if (!string.IsNullOrWhiteSpace(_instrument))
        {
            var sanitizedInstrument = SanitizeFileName(_instrument);
            _jsonlPath = Path.Combine(_robotLogDir, $"robot_{sanitizedInstrument}_{_instanceId}.jsonl");
        }
        else
        {
            _jsonlPath = Path.Combine(_robotLogDir, $"robot_skeleton_{_instanceId}.jsonl");
        }
    }
    ...
    public void Write(object evt)
    {
        ...
        {
            _loggingService = RobotLoggingService.GetInstance(_projectRoot, _robotLogDir);
        }
                ...
                else
                {
                    ...
                    if (evt is Dictionary<string, object?> evtDict)
                    {
                        ...
                        if (streamVal == "__engine__")
                        {
                            ...
                            EmergencyLogger.WriteEngineFallback(_robotLogDir, diagnosticEvent);
                            return;
                        }
                    }
                }
        ...
        if (isEngineEvent)
        {
            ...
            EmergencyLogger.WriteEngineFallback(_robotLogDir, evt);
            return;
        }
