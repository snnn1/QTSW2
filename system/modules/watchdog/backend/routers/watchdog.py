...
# Directory containing the `modules` package (…/system)
...
from modules.watchdog.run_artifacts import (
    resolve_active_persistence_base,
    read_run_summary_json,
    read_key_events_tail,
    discover_recent_run_summaries,
    compute_stability_label,
    validate_peek_run_root,
)
...
async def get_run_summary(
    ...
    """
    Read-through: engine summary.json from active persistence root (no inference).
    Resolution: WATCHDOG_PERSISTENCE_BASE → runs/LATEST_RUN.txt → WATCHDOG_FALLBACK_ROOT.
    """
    root: Optional[Path] = None
    if run_root:
        root = validate_peek_run_root(run_root.strip())
        if root is None:
            return {"available": False, "reason": "INVALID_RUN_ROOT"}
    else:
...
async def get_key_events(
    ...
    run_root: Optional[str] = Query(
        None,
        description="Optional peek: same as GET /run-summary run_root.",
    ),
):
    """Last N lines from KEY_EVENTS.jsonl under the same active persistence root (parsed JSON objects)."""
    root: Optional[Path] = None
    if run_root:
        root = validate_peek_run_root(run_root.strip())
        if root is None:
            return {"available": False, "reason": "INVALID_RUN_ROOT", "events": []}
    else:
        root = resolve_active_persistence_base()
    if root is None:
        return {"available": False, "reason": "NO_ACTIVE_RUN_ROOT", "events": []}
    events = read_key_events_tail(root, limit=limit)
    return {"available": True, "events": events, "persistence_root": str(root)}


@router.get("/recent-runs")
async def get_recent_runs(limit: int = Query(5, ge=1, le=20)):
    """Newest summary.json snapshots (display-only aggregation)."""
    rows = discover_recent_run_summaries(limit=limit)
    summaries = [s for _, s in rows]
    stability = compute_stability_label(summaries)
    runs_out = [{"summary_path": str(path), "summary": sm} for path, sm in rows]
    return {"stability": stability, "runs": runs_out}
