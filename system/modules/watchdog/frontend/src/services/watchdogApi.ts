...
/** Engine summary.json read-through (active run or optional peek run_root). */
export async function fetchRunSummary(runRoot?: string | null): Promise<ApiResponse<RunSummaryResult>> {
  try {
    const qs = new URLSearchParams()
    if (runRoot) qs.set('run_root', runRoot)
    const url = qs.toString() ? `${API_BASE}/run-summary?${qs}` : `${API_BASE}/run-summary`
    const response = await fetchWithTimeout(url)
    if (!response.ok) {
      return { data: null, error: `HTTP ${response.status}: ${response.statusText}` }
    }
    const data = (await response.json()) as RunSummaryResult
    return { data, error: null }
  } catch (error) {
    return { data: null, error: error instanceof Error ? error.message : 'Unknown error' }
  }
...
/** Last N KEY_EVENTS.jsonl lines for active or peek run directory. */
export async function fetchKeyEvents(
  limit: number,
  runRoot?: string | null
): Promise<ApiResponse<KeyEventsResponse>> {
  try {
    const qs = new URLSearchParams({ limit: String(limit) })
    if (runRoot) qs.set('run_root', runRoot)
    const response = await fetchWithTimeout(`${API_BASE}/key-events?${qs}`)
    if (!response.ok) {
      return { data: null, error: `HTTP ${response.status}: ${response.statusText}` }
    }
...
/** Newest summary.json snapshots + display-only stability label. */
export async function fetchRecentRuns(limit = 5): Promise<ApiResponse<RecentRunsResponse>> {
  try {
    const response = await fetchWithTimeout(`${API_BASE}/recent-runs?limit=${limit}`)
    if (!response.ok) {
      return { data: null, error: `HTTP ${response.status}: ${response.statusText}` }
    }
