  ...
  try {
    switch (type) {
      ...
      case WORKER_MESSAGE_TYPES.CALCULATE_TIMETABLE: {
        // Execution timetable is API-only (GET /api/timetable/current). Not computed here.
        const { requestId } = payload
        console.debug('[matrixWorker] CALCULATE_TIMETABLE: no-op (not used for execution grid)')
        self.postMessage({
          type: WORKER_RESPONSE_TYPES.TIMETABLE,
          payload: { timetable: [], requestId },
        })
        break
      }
