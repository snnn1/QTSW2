...
export function usePipelineState() {
  ...
  const updatePipelineStatus = useCallback((statusData, source = 'unknown') => {
    ...
    setPipelineStatus(prev => {
      ...
      // Clear "starting" spinner whenever the pipeline is not running — includes success (idle),
      // failure (error), and stop (stopped). Previously only idle/success cleared it, so failed
      // runs left the UI stuck looking "running".
      if (!isRunning) {
        setIsStarting(false)
