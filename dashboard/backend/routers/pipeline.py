...
async def reset_pipeline():
    """
    Reset pipeline state (clear current run and stale locks).
    """
    orchestrator_instance = get_orchestrator()
    if orchestrator_instance is None:
        raise HTTPException(
    ...
    try:
        ...
        lock_file = orchestrator_instance.config.lock_dir / "pipeline.lock"
        if lock_file.exists():
            try:
                lock_file.unlink()
        ...
        return {"message": "Pipeline state and locks reset"}
    except Exception as e:
